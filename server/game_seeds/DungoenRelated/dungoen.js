...
const monsterMap = Object.fromEntries(
  (await Monster.find({ slug: { $in: ['echo-bat', 'sloth-slime', 'hollow-child', 'false-prophet'] } }))
    .map(m => [m.slug, m])
);

const eventList = await Event.find({ slug: { $in: ['strange-whispers', 'golden-chest', 'wandering-merchant', 'spike-trap', 'mirror-of-self'] } });
...
for (let i = 1; i <= 10; i++) {
  ...
  const floorEvents = getRandom(eventList, Math.random() < 0.6 ? 1 : 0).map(e => e._id);
