...
export const resolveEventEffects = async (event, stats, currentHp) => {
  try {
    console.log(
      `Processing event: ${event.type || "unknown"} - ${
        event.name || "unnamed"
      }`
    );
    ...
    try {
      ...
      if (effect.dungeonExp) {
        ...
        if (!isNaN(expAmount)) {
          ...
          console.log(
            `Added ${expAmount} experience, new total: ${stats.dungeonExp}`
          );
        }
      }
        ...
        for (const key in effect.statChange) {
          ...
          if (!isNaN(changeAmount) && stats.assignedStats) {
            ...
            if (!isNaN(newValue)) {
              ...
              console.log(
                `Changed ${key} by ${changeAmount}, new value: ${newValue}`
              );
            }
          }
        }
      ...
      if (
        effect.itemDrop &&
        Array.isArray(effect.itemDrop) &&
        effect.itemDrop.length > 0
      ) {
        // 可扩展：随机掉落实现
    ...
    switch (event.type) {
      ...
      case "trap":
        ...
        const damageAmount = Math.floor(
          (stats.assignedStats?.hp || 100) * 0.15
        ); // 造成15%最大生命值的伤害
