...
const CLASS_COMBAT_CONFIG = {
  warrior: {
    ...
    reduceDamage: (damage, stats) => {
      return Math.max(
        1,
        damage - Math.floor(stats.assignedStats?.defense * 0.35)
      );
    },
  },
  mage: {
    ...
    reduceDamage: (damage, stats) => {
      return Math.max(
        1,
        damage - Math.floor(stats.assignedStats?.defense * 0.2)
      );
    },
  },
  rogue: {
    ...
    reduceDamage: (damage, stats) => {
      return Math.max(
        1,
        damage - Math.floor(stats.assignedStats?.defense * 0.25)
      );
    },
  },
  archer: {
...
export const executeCombat = async (monsterIds, stats, currentHp) => {
  ...
  // 如果没有classSlug，尝试从className推断
    ...
    if (classNameLower.includes("mage") || classNameLower.includes("法师")) {
      ...
      classNameLower.includes("rogue") ||
      classNameLower.includes("盗贼")
    ) {
      playerClass = "rogue";
      console.log(`Inferred 'rogue' class from className: ${stats.className}`);
    } else if (
      classNameLower.includes("archer") ||
      classNameLower.includes("弓手")
    ) {
      playerClass = "archer";
      console.log(`Inferred 'archer' class from className: ${stats.className}`);
    } else {
      console.log(
        `Using default 'warrior' class (couldn't infer from: ${stats.className})`
      );
    }
  } else {
    console.warn(
      "No class information found in stats, using default warrior class"
    );
  }

  const classConfig =
    CLASS_COMBAT_CONFIG[playerClass] || CLASS_COMBAT_CONFIG.warrior;
  ...
  // 查找所有相关怪物
  const monsters = await Monster.find({ _id: { $in: monsterIds } }).populate(
    "skills"
  );
  ...
  if (playerClass === "rogue" && classConfig.getEvasionBonus) {
    ...
    console.log(
      `Rogue evasion bonus: +${classConfig
        .getEvasionBonus(stats)
        .toFixed(1)}% (Total: ${playerEvasion.toFixed(1)}%)`
    );
  }
  ...
  for (const monster of monsters) {
    ...
    console.log(
      `Monster stats: HP=${mHp}, ATK=${mAttack}, DEF=${mDefense}, M.RES=${mMagicResist}, SPD=${mSpeed}, CRIT=${mCritRate}%, EVA=${mEvasion}%`
    );
    ...
    while (monsterCurrentHp > 0 && hp > 0) {
      ...
      if (playerTurn) {
        ...
        if (
          playerClass === "archer" &&
          isFirstPlayerAttack &&
          classConfig.getFirstAttackCritBonus
        ) {
          critChance += classConfig.getFirstAttackCritBonus();
          console.log(
            `Archer first attack bonus: +${classConfig.getFirstAttackCritBonus()}% crit chance (Total: ${critChance}%)`
          );
        }
        ...
        if (isCritical) {
          ...
          logs.push(
            `CRITICAL! 🗡️ 你攻击了 ${mName}，造成了 ${playerDamage} 点伤害！`
          );
        } else {
        ...
        if (playerClass === "mage") {
          ...
          const magicResistReduction = Math.floor(
            playerDamage * (mMagicResist / 200)
          ); // 除以200而不是100，减轻抗性效果
          finalDamage = Math.max(1, playerDamage - magicResistReduction);
          if (magicResistReduction > 0) {
            console.log(
              `Monster magic resist reduced damage: ${playerDamage} -> ${finalDamage}`
            );
          ...
          if (isCritical) {
            ...
            if (penetrationBonus > 0) {
              ...
              console.log(
                `Critical hit magic penetration bonus: +${penetrationBonus} damage`
              );
          ...
          if (defenseReduction > 0) {
            console.log(
              `Monster defense reduced damage: ${playerDamage} -> ${finalDamage}`
            );
          }
        }
        ...
        console.log(
          `Monster HP: ${monsterCurrentHp + finalDamage} -> ${monsterCurrentHp}`
        );
        ...
        if (isEvaded) {
          console.log(
            `Player evaded the attack! (${playerEvasion.toFixed(1)}% chance)`
          );
          logs.push(`EVADE! 👹 ${mName} 的攻击被你闪避了！`);
        } else {
          // 技能或攻击处理（选最高优先级技能）
          const usableSkills = (monster.skills || []).filter(
            (s) => s.effect === "dealDamage"
          );
          console.log(
            `Monster has ${usableSkills.length} usable damage skills`
          );

          const selectedSkill = usableSkills.sort(
          ...
          if (selectedSkill) {
            ...
            console.log(
              `Skill damage calculation: ${selectedSkill.effectValue} * 0.9 = ${damage}`
            );
            logs.push(
              `${mName} used ${selectedSkill.name}, dealt ${damage} damage!`
            );
          } else {
            // 普通攻击
            ...
            if (isMonsterCrit) {
              ...
              logs.push(
                `CRITICAL! 👹 ${mName} 攻击了你，造成了 ${damage} 点伤害！`
              );
            } else {
          ...
          if (damageReduction > 0) {
            console.log(
              `Damage reduced by defense: ${damage} -> ${reducedDamage} (${damageReduction} blocked)`
            );
  ...
  console.log(
    `Updated player stats: EXP ${oldExp} -> ${stats.dungeonExp}, Gold ${oldGold} -> ${stats.gold}`
  );
