  ...
  try {
    ...
    if (task.type === "long") {
      ...
      for (const subTask of pendingSubTasks) {
        ...
        console.log(`自动完成子任务: ${subTask.title}, 获得 ${experience} XP, ${gold} Gold`);
      }
      ...
      const { experience: calculatedFinalExp, gold: calculatedFinalGold } = calculateReward(
        bonusExp,
        bonusGold,
        task.cardUsed?.bonus
      );
      ...
      if (pendingSubTasks.length === 0 && completedSubTasks.length > 0) {
        ...
        console.log("总计奖励 - XP:", totalExp, "(子任务:", pendingSubTasksExp, "+ 任务奖励:", finalExp, ")");
        console.log("总计奖励 - Gold:", totalGold, "(子任务:", pendingSubTasksGold, "+ 任务奖励:", finalGold, ")");
      }
      ...
      if (!task.completedAt) {
        task.completedAt = new Date();
        console.log(`Setting short task ${taskId} completedAt to ${task.completedAt}`);
      }
    ...
    console.log(`奖励已发放 - 用户: ${userId}, 获得 ${totalExp} XP, ${totalGold} Gold`);
    ...
    if (task.type === "long") {
      ...
      const alreadyCompletedSubTasksCount = task.subTasks.filter(st => st.status === "completed" && st.completedAt < task.completedAt).length;
