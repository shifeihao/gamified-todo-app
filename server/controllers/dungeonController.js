...
function validateUserStats(stats) {
  ...
  if (stats.currentExploration) {
    if (isNaN(stats.currentExploration.currentHp)) {
      console.warn(`Fixing invalid currentHp: ${stats.currentExploration.currentHp}`);
      stats.currentExploration.currentHp = 100;
    }
    
    if (isNaN(stats.currentExploration.floorIndex)) {
      console.warn(`Fixing invalid floorIndex: ${stats.currentExploration.floorIndex}`);
...
export const enterDungeon = async (req, res) => {
  try {
    ...
    // 查找迷宫
    const dungeon = await Dungeon.findOne({ slug: dungeonSlug, isActive: true });
    if (!dungeon) {
...
export const exploreCurrentFloor = async (req, res) => {
  try {
    ...
    if (!dungeonSlug) {
      console.log('Missing dungeonSlug in exploration data, trying to use user stats dungeonSlug');
      ...
      if (dungeonSlug) {
        ...
        await stats.save();
        console.log('Updated currentExploration.dungeonSlug with:', dungeonSlug);
      } else {
    ...
    console.log(`Finding dungeon with slug: ${dungeonSlug}, floor: ${floorIndex}`);
    ...
    console.log(`Dungeon found: ${dungeon.name}, total floors: ${dungeon.floors?.length || 0}`);
    ...
    if (!floor) {
      ...
      if (dungeon.floors && dungeon.floors.length > 0) {
        console.log('Available floor indices:', dungeon.floors.map(f => f.floorIndex));
      ...
      for (const event of floor.events) {
        ...
        try {
          ...
          if (result?.hp != null && !isNaN(result.hp)) {
            ...
            console.warn('Invalid HP value from event, keeping current HP:', hp);
          }
    ...
    if (Array.isArray(floor.monsters)) {
      ...
      for (const monsterInfo of floor.monsters) {
        if (monsterInfo?.monster?._id) {
          ...
          console.log(`Adding monster ${monsterInfo.monster.name || 'Unknown'} x${count}`);
    ...
    // 检查是否达到检查点
    const nextFloor = dungeon.floors?.find(f => f.floorIndex === stats.currentExploration.floorIndex);
    if (nextFloor?.checkpoint) {
      console.log(`Reached checkpoint at floor ${stats.currentExploration.floorIndex}`);
      stats.checkpointFloor = stats.currentExploration.floorIndex;
      stats.currentExploration.status = stats.currentExploration.status || {};
      stats.currentExploration.status.atCheckpoint = true;
      logs.push(`You have reached a checkpoint at floor ${stats.currentExploration.floorIndex}!`);
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Failed to save exploration progress' });
    }
...
export const continueExploration = async (req, res) => {
  try {
    ...
    const stats = await UserDungeonStats.findOne({ user: userId });
    if (!stats || !stats.currentExploration) {
      return res.status(400).json({ error: 'No active exploration to continue' });
    }
...
export const prepareCombatAfterShop = async (req, res) => {
  try {
    ...
    let dungeonSlug = stats.currentExploration.dungeonSlug || stats.dungeonSlug || 'echo-labyrinth';
    let floorIndex = stats.currentExploration.floorIndex;
    floorIndex = parseInt(floorIndex, 10);
...
export const updateCombatResult = async (req, res) => {
  try {
    ...
    let floorIndex = stats.currentExploration.floorIndex;
    const dungeonSlug = stats.currentExploration.dungeonSlug || stats.dungeonSlug;
    
    console.log(`Combat result update - Floor before: ${floorIndex}, Survived: ${survived}`);
    ...
    if (survived) {
      ...
      console.log(`Combat result update - Floor after: ${stats.currentExploration.floorIndex}`);
      
      // 检查是否达到迷宫终点
      const isEnd = dungeon && stats.currentExploration.floorIndex > (dungeon.maxFloor || 1);
      if (isEnd) {
        // 结束迷宫探索逻辑...
        stats.currentExploration = undefined;
